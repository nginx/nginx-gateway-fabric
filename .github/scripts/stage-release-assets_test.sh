#!/usr/bin/env bash
#
# Tests for stage-release-assets.sh, against a stubbed az.
#
# What is pinned: that every inventoried file is uploaded to the path the
# manifest will record, that the recorded sha256 is the file's, that an
# incomplete inventory stages nothing, and that a failed upload fails the
# run rather than leaving a manifest that names a blob which is not there.
#
# Run directly: bash .github/scripts/stage-release-assets_test.sh
