...
# the build files.
#
# Why this exists: these versions are pinned in eleven places across four
# Dockerfiles and one Go source file, and a release bumps most of them. Doing
# it by hand is how a release ends up with the Alpine and UBI images running
# different versions of NGINX Plus. The F5 WAF module version is the sharpest
# case: it appears in two different formats, dot-separated for apk and
# plus-separated for rpm, so the same number has to be written two ways.
#
# The agent version is deliberately not release-driven. Agent releases are
# public ahead of ours and its bumps follow main like any other dependency, so
# it is included here only for completeness and is never required.
#
# Usage:
...
#                        The rpm format, 37.1+5.715, is derived from it.
...
# ---------------------------------------------------------------------------
# Validation. A malformed version written into a Dockerfile fails at image
# build time at the earliest, and can silently select a different package at
# worst, so reject it here.
# ---------------------------------------------------------------------------
validate() {
...
# The WAF module version is given once, in apk format, and rendered two ways.
NAP_WAF_MODULE_RPM=""
