...
#
# The store is untrusted by design; the manifest is what is trusted. So the
# cases that matter are the ones where the store serves the wrong thing and
# the script has to notice: a tampered archive, a tampered checksums file, a
# missing bundle, a manifest entry that cannot be checked.
#
# Run directly: bash .github/scripts/fetch-release-assets_test.sh
...
# The manifest matches the store, but the store's checksums file disagrees
# with the archives it lists -- i.e. prep's own checksums were wrong. The
# third check exists for exactly this: the two signed artifacts must agree.
...
# --- the manifest cannot be trusted to check ---------------------------------
