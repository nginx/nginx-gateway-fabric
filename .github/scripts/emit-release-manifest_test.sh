#!/usr/bin/env bash
# Tests for emit-release-manifest.sh. Exit status: 0 all passed, 1 any failed.
...
# Each of these would otherwise produce a manifest that publish accepts and misreads.
...
# publish's merge-back gate compares the tree hash; a malformed one disables it.
d="$(digest_dir source)"
...
# publish promotes by digest so a moved tag cannot change what ships.
d="${TMP_ROOT}/tagref"
mkdir -p "${d}"
cat >"${d}/ngf.json" <<EOF
...
# Filenames are deliberately opposite the image name order, so only sorting
# by content (not `find | sort`) gives ngf first.
d="${TMP_ROOT}/order1"
mkdir -p "${d}"
