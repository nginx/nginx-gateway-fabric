...
# ---------------------------------------------------------------------------
# The manifest handoff. gh is stubbed per subcommand; the functions are the
# sourced ones, run in subshells because `fail` exits.
# ---------------------------------------------------------------------------
FIX="${TMP}/fixture"
mkdir -p "${FIX}"
...
"api repos/"*"/actions/artifacts"*)
    # The --jq the script passes is applied here, as gh would.
    jq -r "${4}" <"${GH_ARTIFACTS}"
    ;;
"run download")
...
# Three artifacts for the version: an old one, the newest that is still
# downloadable, and a newer one that has expired. The middle one must win.
