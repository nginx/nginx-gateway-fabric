...
#   SIGNER_REPOSITORY  owner/name of the repository whose prep signed it. Required.
...
# Provenance, checked before contents are read: a manifest that fails this is
# not a release artifact, whatever it says inside. Identity is anchored to the
# signer repository, workflow file, and an internal release branch.
escape_re() {
    printf '%s' "$1" | sed 's/[.]/\\./g'
}
