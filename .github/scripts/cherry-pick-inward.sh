...
#
# Why this direction: the public `cherry-pick.yml` fires on a merged pull
# request and pushes into the release branch. That cannot reach the internal
# mirror without a credential in the public repository that can write to it,
# which is the one credential this design refuses to create -- it would unlock
# embargoed fixes before disclosure. Running the transfer from the mirror
# inverts it: reading public data needs no privileged credential at all, and
# the write uses the mirror's own token.
#
# It opens a pull request rather than pushing, so a cherry-pick that applies
# cleanly but is wrong still gets read by someone.
#
# Usage:
...
# True when the branch already carries the given pull request.
#
# Two ways it can: the squash commit's subject ends "(#N)", which is how this
# repository merges, or a previous run left cherry-pick's "-x" trailer naming
# the commit. Checking both means a pick is not repeated because someone
# reworded a subject, and that an unlabelled-then-labelled pull request is not
# applied twice.
already_applied() {
...
# Merged pull requests carrying the label, oldest merge first.
#
# Order matters: picking them in the order they landed on main is what keeps
# a later fix applying on top of an earlier one rather than conflicting with
# it.
list_candidates() {
    "${GH}" pr list \
        --repo "${PUBLIC_REPO}" \
