...
var _ = Describe("AuthenticationFilter", Ordered, Label("functional", "authentication-filter"), func() {
	var (
		files = []string{
			"authentication-filter/basic/cafe.yaml",
			"authentication-filter/basic/gateway.yaml",
			"authentication-filter/basic/grpc-backend.yaml",
		}
	...
	// When("invalid key AuthenticationFilters are applied to the resources", func() {
	// 	var (
	// 		invalidAuthenticationFilters = []string{
	// 			"authentication-filter/basic/invalid-key-auth.yaml",
	// 		}
	// 	)
	...
	// 	Specify("authenticationFilters are accepted", func() {
	// 		validAuthenticationFilterNames := []string{
	// 			"basic-auth2",
	// 		}
	...
	// 		for _, name := range validAuthenticationFilterNames {
	// 			nsname := types.NamespacedName{Name: name, Namespace: namespace}
	...
	// 				WithTimeout(timeoutConfig.GetStatusTimeout).
	// 				WithPolling(500*time.Millisecond).
	// 				Should(Succeed(), fmt.Sprintf("%s was accepted", name))
	// 		}
	...
	When("invalid AuthenticationFilters are applied to the resources", func() {
		var (
			...
			validAuthenticationFilter = []string{
				"authentication-filter/basic/basic-auth1.yaml",
			}
		...
		Specify("authenticationFilters are accepted", func() {
			...
			invalidAuthenticationFilter := "basic-auth-opaque"
			...
			Eventually(checkForAuthenticationFilterToBeAccepted).
				WithArguments(
					types.NamespacedName{Name: invalidAuthenticationFilter, Namespace: namespace},
				).
				WithTimeout(timeoutConfig.GetStatusTimeout).
				WithPolling(500*time.Millisecond).
				ShouldNot(Succeed(), fmt.Sprintf("%s was accepted", invalidAuthenticationFilter))
