				...
				It("should successfully authenticate with valid JWT token", func() {
					Eventually(
						func() error {
							return framework.ExpectRequestToSucceed(
								timeoutConfig.RequestTimeout,
								framework.GetURL("http://cafe.example.com/jwt-coffee", port),
								address,
								"URI: /jwt-coffee",
								framework.WithRequestHeaders(map[string]string{
				...
				It("should return 500 when JWT filter references non-existent secret", func() {
					Eventually(
						func() error {
							return framework.Expect500Response(
								timeoutConfig.RequestTimeout,
								framework.GetURL("http://cafe.example.com/jwt-invalid-secret", port),
								address,
								framework.WithRequestHeaders(map[string]string{
				...
				It("should return 401 when JWT filter uses secret with invalid JWKS data", func() {
					Eventually(
						func() error {
							return framework.Expect401Response(
								timeoutConfig.RequestTimeout,
								framework.GetURL("http://cafe.example.com/jwt-invalid-jwks", port),
								address,
								framework.WithRequestHeaders(map[string]string{
									"Authorization": "Bearer some.jwt.token",
								}))
				...
				It("should successfully authenticate with valid JWT token from Keycloak", func() {
					Eventually(
						func() error {
							return framework.ExpectRequestToSucceed(
								timeoutConfig.RequestTimeout,
								framework.GetURL("http://cafe.example.com/jwt-remote-coffee", port),
								address,
				...
				It("should allow access to unprotected endpoint without authentication", func() {
					Eventually(
						func() error {
							return framework.ExpectRequestToSucceed(
								timeoutConfig.RequestTimeout,
								framework.GetURL("http://cafe.example.com/jwt-remote-tea", port),
								address,
								"URI: /jwt-remote-tea",
							)
						}).
				...
				It("should return 200 when Keycloak token has matching claim", func() {
					...
					Eventually(
						func() error {
							return framework.ExpectRequestToSucceed(
								timeoutConfig.RequestTimeout,
								framework.GetURL("http://cafe.example.com/jwt-remote-authz", port),
								address,
				...
				It("should return 200 when token has matching claim value", func() {
					...
					Eventually(
						func() error {
							return framework.ExpectRequestToSucceed(
								timeoutConfig.RequestTimeout,
								framework.GetURL("http://cafe.example.com/jwt-authz-any", port),
								address,
								"URI: /jwt-authz-any",
								framework.WithRequestHeaders(map[string]string{
				...
				It("should return 200 when token satisfies ALL required claims", func() {
					...
					Eventually(
						func() error {
							return framework.ExpectRequestToSucceed(
								timeoutConfig.RequestTimeout,
								framework.GetURL("http://cafe.example.com/jwt-authz-all", port),
								address,
								"URI: /jwt-authz-all",
								framework.WithRequestHeaders(map[string]string{
									"Authorization": fmt.Sprintf("Bearer %s", token),
				...
				It("should return 200 when claim value matches regex", func() {
					...
					Eventually(
						func() error {
							return framework.ExpectRequestToSucceed(
								timeoutConfig.RequestTimeout,
								framework.GetURL("http://cafe.example.com/jwt-authz-regex", port),
								address,
								"URI: /jwt-authz-regex",
								framework.WithRequestHeaders(map[string]string{
									"Authorization": fmt.Sprintf("Bearer %s", token),
				...
				It("should return 200 when token matches first rule", func() {
					...
					Eventually(
						func() error {
							return framework.ExpectRequestToSucceed(
								timeoutConfig.RequestTimeout,
								framework.GetURL("http://cafe.example.com/jwt-authz-multi", port),
								address,
								"URI: /jwt-authz-multi",
								framework.WithRequestHeaders(map[string]string{
									"Authorization": fmt.Sprintf("Bearer %s", token),
				...
				It("should return 200 when token satisfies all rules", func() {
					...
					Eventually(
						func() error {
							return framework.ExpectRequestToSucceed(
								timeoutConfig.RequestTimeout,
								framework.GetURL("http://cafe.example.com/jwt-authz-multi-all", port),
								address,
				...
				It("should return 200 and forward claim values as headers to backend", func() {
					...
					Eventually(
						func() error {
							return framework.ExpectRequestToSucceed(
								timeoutConfig.RequestTimeout,
								framework.GetURL("http://cafe.example.com/jwt-authz-header", port),
								address,
