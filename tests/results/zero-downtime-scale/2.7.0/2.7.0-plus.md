...
- New connection refused errors and significant latency spikes (mean ~700ms, max ~20s) observed in the gradual scale up test on multiple-pods-per-node, a regression compared to 2.6.0 which had no dropped traffic in that scenario.
- Latency for all other tests is comparable to 2.6.0.
