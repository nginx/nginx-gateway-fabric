...
- Sections of dropped traffic when abruptly scaling with NGINX Plus. First seen in 2.3 results, for the most part the same tests see the same dropped traffic, only occurring on http traffic. This is not seen on oss results.
