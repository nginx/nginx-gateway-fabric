...
func TestProcessPolicies(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			processed, _ := processPolicies(test.policies, test.validator, routes, services, gateways, &fetch.DefaultFetcher{})
...
func TestProcessPolicies_RouteOverlap(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			processed, _ := processPolicies(test.policies, test.validator, test.routes, nil, gateways, &fetch.DefaultFetcher{})
...
func TestFetchPolicyBundleData(t *testing.T) {
	...
	nonWAFPolicyGVK := schema.GroupVersionKind{
		Group:   ngfAPIv1alpha2.SchemeGroupVersion.Group,
		Version: ngfAPIv1alpha2.SchemeGroupVersion.Version,
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			// Set up fake responses for the new method with options
			fakeFetcher.GetRemoteFileStub = func(url string, _ ...fetch.Option) ([]byte, error) {
				switch url {
			...
			result := fetchPolicyBundleData(test.processedPolicies, fakeFetcher)
