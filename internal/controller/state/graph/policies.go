...
func processPolicies(
...
) (map[PolicyKey]*Policy, map[SHA256FileName]*WAFBundleData) {
	...
	refPolicyBundles := fetchPolicyBundleData(processedPolicies, fetcher)
...
func fetchPolicyBundleData(
	...
	fetcher fetch.Fetcher,
) map[SHA256FileName]*WAFBundleData {
	refPolicyBundles := make(map[SHA256FileName]*WAFBundleData)
	...
	for policyKey, policy := range processedPolicies {
		...
		if wafPolicy.Spec.PolicySource != nil {
			...
			if !fetchAndStoreBundle(fileLoc, wafPolicy.Spec.PolicySource, fetcher, policy, refPolicyBundles) {
		...
		for _, secLog := range wafPolicy.Spec.SecurityLogs {
			...
			if !fetchAndStoreBundle(logFileLoc, secLog.LogProfileBundle, fetcher, policy, refPolicyBundles) {
...
func fetchAndStoreBundle(
	...
	fetcher fetch.Fetcher,
	policy *Policy,
	bundles map[SHA256FileName]*WAFBundleData,
) bool {
	if fileLocation == "" {
	...
	data, err := fetcher.GetRemoteFile(fileLocation, options...)
	if err != nil {
		policy.Valid = false
	...
	hash := sha256.New()
	hash.Write([]byte(fileLocation))

	shaFileLocation := base64.StdEncoding.EncodeToString(hash.Sum(nil))
	key := SHA256FileName(shaFileLocation)
	bundles[key] = &bundleData
