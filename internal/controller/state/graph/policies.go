...
func processPolicies(
	...
	refPolicyBundles := fetchPolicyBundleData(processedPolicies)
...
func fetchPolicyBundleData(
	processedPolicies map[PolicyKey]*Policy,
) map[WAFBundleKey]*WAFBundleData {
	return fetchPolicyBundleDataWithFetcherFactory(processedPolicies, nil)
}

func fetchPolicyBundleDataWithFetcherFactory(
	processedPolicies map[PolicyKey]*Policy,
	fetcherFactory func([]fetch.Option) fetch.Fetcher,
) map[WAFBundleKey]*WAFBundleData {
	...
	for policyKey, policy := range processedPolicies {
		...
		if wafPolicy.Spec.PolicySource != nil {
			fileLoc := wafPolicy.Spec.PolicySource.FileLocation
			if !fetchAndStoreBundle(fileLoc, wafPolicy.Spec.PolicySource, policy, refPolicyBundles, fetcherFactory) {
				continue // Policy was marked invalid, skip security logs
			}
		}

		for _, secLog := range wafPolicy.Spec.SecurityLogs {
			if secLog.LogProfileBundle == nil {
				continue
			}

			logFileLoc := secLog.LogProfileBundle.FileLocation
			if !fetchAndStoreBundle(logFileLoc, secLog.LogProfileBundle, policy, refPolicyBundles, fetcherFactory) {
...
// It allows injecting a custom fetcher factory for testing purposes.
...
func fetchAndStoreBundle(
	...
	if fileLocation == "" {
		return true // Empty location is not an error, just skip
	}

	// Build fetch options from policy source configuration
	options := buildFetchOptions(policySource)

	// Create a fetcher configured for this specific policy source
	var fetcher fetch.Fetcher
	if fetcherFactory != nil {
		fetcher = fetcherFactory(options)
	} else {
		fetcher = fetch.NewDefaultFetcher(options...)
	}
