...
func TestGenerateExternalAuthInternalPath(t *testing.T) {
	t.Parallel()
	g := NewWithT(t)

	result := generateExternalAuthInternalPath(
		types.NamespacedName{Namespace: "ns", Name: "route"},
		2,
	)
	g.Expect(result).To(Equal("/_ngf-internal-ext-auth-ns_route_rule2"))
}
