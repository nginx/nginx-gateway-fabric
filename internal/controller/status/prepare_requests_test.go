...
func TestBuildNGFPolicyStatusesProgrammedCondition(t *testing.T) {
	...
	// The GEP-713 "Programmed" condition is added based on the policy Kind, so each of the NGF settings
	// policy kinds is exercised. ClientSettingsPolicy is used as the representative Source type for the
	// fake client (status assembly only branches on the PolicyKey Kind, not the concrete Source type).
	policyKinds := []string{
