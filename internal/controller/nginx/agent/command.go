...
func (cs *commandService) Subscribe(in pb.CommandService_SubscribeServer) error {
	...
	for {
		...
		case msg := <-channels.ListenCh:
			...
			// Track the pending request so we can handle connection resets properly
			pendingRequest = &msg
		case err = <-msgr.Errors():
			cs.logger.Error(err, "connection error", "pod", conn.PodName)
			deployment.SetPodErrorStatus(conn.PodName, err)

			// Don't signal completion if we have a pending request that hasn't been processed
			// The connection error means the operation failed, not that it completed
			if pendingRequest == nil {
				...
				cs.logger.V(1).Info("Connection error during pending request, not signaling completion")
			}
