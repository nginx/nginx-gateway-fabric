...
// existingPathSet returns the set of paths that route rules will occupy in the NGINX config,
// including derived locations for non-slashed PathPrefix rules (e.g. /coffee also claims /coffee/).
...
// It is only created when postLogoutURI is a path-only URI.
