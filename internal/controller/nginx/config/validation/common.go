...
func validatePathInRegexMatch(path string) error {
	...
	if _, err := regexp.Compile(path); err != nil {
		return errors.New(fmt.Sprintf("invalid RE2 regex for path '%s': %v", path, err))
	}

	for i := 0; i < len(path); i++ {
		if path[i] == '$' && (i == 0 || path[i-1] != '\\') {
			return errors.New(fmt.Sprintf("invalid unescaped `$` at position %d in path '%s'", i, path))
		}
	}
	...
	for _, la := range lookarounds {
		if strings.Contains(path, la) {
			return errors.New(fmt.Sprintf("lookahead/lookbehind '%s' found in path '%s' which is not supported in RE2", la, path))
		}
	}
	...
	if len(matches) > 0 {
		...
		return errors.New(fmt.Sprintf("backreference(s) %v found in path '%s' which are not supported in RE2", positions, path))
	}
