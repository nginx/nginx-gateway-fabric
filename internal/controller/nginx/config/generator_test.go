...
func TestGenerate(t *testing.T) {
	...
	*/
	...
	g.Expect(files[12].Meta.Name).To(Equal("/etc/nginx/secrets/jwt_auth_default_auth-jwt-user"))
	AuthSecrets = string(files[12].Contents)
	g.Expect(AuthSecrets).To(Equal("token"))
	...
	certBundle := string(files[17].Contents)
