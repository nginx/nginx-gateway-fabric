...
func TestTransformSecret(t *testing.T) {
	...
	secret := &corev1.Secret{
		Data: map[string][]byte{
			...
			secrets.DataplaneSecretKey: []byte(secrets.DataplaneSecretKey),
			...
			secrets.GuardrailsTokenKey: []byte("guardrails-token"),
	...
	g.Expect(resSecret.Data).To(HaveKey(secrets.DataplaneSecretKey))
	...
	g.Expect(resSecret.Data).To(HaveKey(secrets.GuardrailsTokenKey))
