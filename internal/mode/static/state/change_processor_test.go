	...
	Describe("Normal cases of processing changes", func() {
		...
		Describe("Process gateway resources", Ordered, func() {
			var (
				...
				hr1, hr1Updated                      *v1.HTTPRoute
				gr1, gr1Updated                      *v1.GRPCRoute
				tr1, tr1Updated                      *v1alpha2.TLSRoute
				...
				httpRouteKey1, grpcRouteKey1         graph.RouteKey
				// httpRouteKey2                        graph.RouteKey
				trKey1                        graph.L4RouteKey
			...
			When("the second Gateway is upserted", func() {
				It("returns populated graph using first gateway", func() {
					...
					httpRoute := expGraph.Routes[httpRouteKey1]
					httpRoute.Conditions = append(httpRoute.Conditions,
						staticConds.NewRouteBackendRefRefBackendNotFound(
							"spec.rules[0].backendRefs[0].name: Not found: \"service\"",
						))

					expGraph.Gateways[types.NamespacedName{Namespace: "test", Name: "gateway-2"}] = &graph.Gateway{
						Source: gw2,
						Listeners: []*graph.Listener{
							{
								Name:       httpListenerName,
