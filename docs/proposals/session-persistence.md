...
sticky cookie <name> [expires=<time>] [domain=<domain>] [path=<path>]
                [secure] [httponly] [maxidle=<time>] [maxlife=<time>];
```
...
domain / path – Restrict cookie scope to the domain and path it is defined for.
secure, httponly – Adds standard cookie security attributes.
