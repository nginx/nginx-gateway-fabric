...
4. Once the release branch pipeline completes, run tests using the `release-X.X-rc` images that are pushed to Github (for example, `release-1.3-rc`).
   ...
   3. Run the IPv6 tests using the `make ipv6-tests` target. This must be run from within the `tests` directory. This script need two arguments. The release version (e.g. `v2.1.0`) and the release image tag (e.g. `release-X.X-rc`).
   For example, when running this script for release 2.1.0, it would look like this: `make ipv6-test RELEASE=2.1.1 RELEASE_IMAGE=release-2.1-rc`
