...
6. Releasing is now two stages, and the CI workflow is not involved in either.
   CI takes no inputs, does not run on `release-*` branches, and cannot create
   a tag or publish anything.

   > **Not yet runnable.** Both stages need repository variables and a token
   > that do not exist yet — see the release owner. The steps below describe
   ...
   **Stage one — prep, in the internal mirror.** Run the `Release Prep`
   ...
   images to the staging registry, runs the functional, conformance and Helm
   suites against **those exact images**, and emits a release manifest. It
   publishes nothing public. Nothing is released if any suite fails.

   **Then merge back.** The internal release branch has to land on the public
   release branch before publish will do anything: publish compares the public
   tree hash against the one recorded in the manifest, and refuses if they
   ...
   **Stage two — publish, in this repository.** Run the `Release Publish`
   ...
   - Create the GitHub release with an autogenerated changelog, attach the
