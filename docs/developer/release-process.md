...
2. **Run the Release PR workflow** with `base-branch: internal/release-X.Y`. That input is required here, because the default derives `release-X.Y` from the version and that is the public branch. Then finish the pull request:
...
2. **Mirror. Long-running tests, against the staged images.** These are not part of prep. Run them from the mirror once prep has staged the images, so they exercise the artifact that will ship rather than a rebuild of it. Each takes `image_source: registry` and `image_registry` set to the NGF repository prep pushed to; the data plane repositories are derived from it. Use `X.Y.Z` for `version` and prep's tag for `image_tag`. The cluster pulls with the registry JWT, which the workflow fetches from the vault for any registry source.
   ...
   - **Longevity:** [Start Longevity Tests](https://github.com/nginx/nginx-gateway-fabric/actions/workflows/longevity-start.yml), then Stop Longevity Tests three days later to collect results and tear down.
   - **NFR:** the [NFR workflow](https://github.com/nginx/nginx-gateway-fabric/actions/workflows/nfr.yml).
   - **GatewayLink:** the [GatewayLink workflow](https://github.com/nginx/nginx-gateway-fabric/actions/workflows/gatewaylink.yml). It stands up a BIG-IP alongside the cluster, so run it once per release rather than per attempt.
...
1. **Land it on `main` first**, with the `needs cherry pick` label, unless it genuinely applies only to the release branch — in which case commit it on `internal/release-X.Y` in the mirror and skip the next step.
...
**Then run prep again.** Anything that changes the internal branch after prep invalidates the images it staged: the manifest records the commit and tree it built, so promote refuses a manifest that no longer matches. Re-run the long-running tests too, or accept that what ships is not what they tested — and if longevity has to be repeated, that is another three days.
