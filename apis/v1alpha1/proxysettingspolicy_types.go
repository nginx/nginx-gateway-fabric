...
type ProxySettingsPolicySpec struct {
	...
	// +kubebuilder:validation:XValidation:message="TargetRefs Kind and Name combination must be unique",rule="self.all(t1, self.exists_one(t2, t1.group == t2.group && t1.kind == t2.kind && t1.name == t2.name))"
	//nolint:lll
