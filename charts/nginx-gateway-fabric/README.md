...
| `nginx.wafContainers` | Configuration for NGINX App Protect WAF v5 containers. These containers are only deployed when WAF is enabled via nginx.config.wafEnabled: true. All settings are optional overrides - defaults are provided by NGF. | object | `{}` |
