...
| `nginxGateway.service` | The service configuration for the NGINX Gateway Fabric control plane. | object | `{"annotations":{},"exposeHealthPort":false,"labels":{}}` |
| `nginxGateway.service.annotations` | The annotations of the NGINX Gateway Fabric control plane service. | object | `{}` |
| `nginxGateway.service.exposeHealthPort` | Expose the readiness probe port on the control plane service. When enabled, the readiness probe endpoint (/readyz) becomes accessible externally, allowing external load balancers (like Azure Application Gateway) to perform health checks. Requires nginxGateway.readinessProbe.enable to be true. | bool | `false` |
