...
![Version: 2.7.2](https://img.shields.io/badge/Version-2.7.2-informational?style=flat-square) ![AppVersion: edge](https://img.shields.io/badge/AppVersion-edge-informational?style=flat-square)
...
| `nginx` | The nginx section contains the configuration for all NGINX data plane deployments installed by the NGINX Gateway Fabric control plane. | object | `{"autoscaling":{"annotations":{},"behavior":{},"enable":false,"maxReplicas":10,"metrics":[],"minReplicas":1,"targetCPUUtilizationPercentage":50,"targetMemoryUtilizationPercentage":50},"config":{},"container":{"hostPorts":[],"lifecycle":{},"readinessProbe":{},"resources":{},"volumeMounts":[]},"debug":false,"image":{"pullPolicy":"Always","repository":"ghcr.io/nginx/nginx-gateway-fabric/nginx","tag":"edge"},"imagePullSecret":"","imagePullSecrets":[],"kind":"deployment","nginxInstanceManager":{"endpointHost":"","endpointPort":4317},"nginxOneConsole":{"dataplaneKeySecretName":"","endpointHost":"agent.connect.nginx.com","endpointPort":443,"skipVerify":false},"patches":[],"plus":false,"pod":{},"podDisruptionBudget":{"enable":false,"maxUnavailable":"","minAvailable":1,"unhealthyPodEvictionPolicy":""},"replicas":1,"service":{"externalTrafficPolicy":"Local","loadBalancerClass":"","loadBalancerIP":"","loadBalancerSourceRanges":[],"nodePorts":[],"patches":[],"type":"LoadBalancer"},"serviceMonitor":{"enable":false,"interval":"","port":"metrics"},"usage":{"caSecretName":"","clientSSLSecretName":"","endpoint":"","enforceInitialReport":true,"resolver":"","secretName":"nplus-license","skipVerify":false},"wafContainers":{}}` |
...
| `nginx.nginxInstanceManager` | Configuration for NGINX Instance Manager. | object | `{"endpointHost":"","endpointPort":4317}` |
| `nginx.nginxInstanceManager.endpointHost` | The Endpoint host that the NGINX Instance Manager telemetry data will be sent to. | string | `""` |
| `nginx.nginxInstanceManager.endpointPort` | The endpoint port that the NGINX Instance Manager telemetry data will be sent to. | int | `4317` |
...
| `nginx.podDisruptionBudget` | PodDisruptionBudget configuration for the NGINX data plane. Only applies when `nginx.kind` is `deployment`. A PodDisruptionBudget is created for every NGINX Deployment provisioned for a Gateway that uses this NginxProxy. | object | `{"enable":false,"maxUnavailable":"","minAvailable":1,"unhealthyPodEvictionPolicy":""}` |
| `nginx.podDisruptionBudget.enable` | Enable or disable PodDisruptionBudget for the NGINX data plane. | bool | `false` |
| `nginx.podDisruptionBudget.maxUnavailable` | Maximum number of pods that can be unavailable after an eviction. Value can be an absolute number (e.g. 1) or a percentage (e.g. 50%). Mutually exclusive with minAvailable. | string | `""` |
| `nginx.podDisruptionBudget.minAvailable` | Minimum number of pods that must be available after an eviction. Value can be an absolute number (e.g. 1) or a percentage (e.g. 50%). Mutually exclusive with maxUnavailable; set this to "" to use maxUnavailable instead. | int | `1` |
| `nginx.podDisruptionBudget.unhealthyPodEvictionPolicy` | Defines when unhealthy pods should be considered for eviction. Valid values are IfHealthyBudget and AlwaysAllow. | string | `""` |
...
| `nginx.serviceMonitor` | Defines the configuration of the serviceMonitor for the nginx data plane. | object | `{"enable":false,"interval":"","port":"metrics"}` |
| `nginx.serviceMonitor.enable` | Enable the serviceMonitor for the data plane. | bool | `false` |
| `nginx.serviceMonitor.interval` | The scraping interval used by the serviceMonitor when scraping metrics. | string | `""` |
| `nginx.serviceMonitor.port` | The name of the port used for scraping metrics. | string | `"metrics"` |
...
| `nginxGateway.leaderElection.leaseDuration` | The duration that non-leader candidates will wait to force acquire leadership. Must be greater than renewDeadline. Must be parsable by https://pkg.go.dev/time#ParseDuration. | string | Defaults to controller-runtime's built-in default (15s) if not set or set to "". |
...
| `nginxGateway.leaderElection.renewDeadline` | The duration that the acting leader will retry refreshing leadership before giving up. Must be greater than retryPeriod * 1.2 and less than leaseDuration. Increase this value if leader election is failing due to slow Kubernetes API server responses. Must be parsable by https://pkg.go.dev/time#ParseDuration. | string | Defaults to controller-runtime's built-in default (10s) if not set or set to "". |
| `nginxGateway.leaderElection.retryPeriod` | The duration the leader election clients should wait between action tries. Must be less than renewDeadline. Must be parsable by https://pkg.go.dev/time#ParseDuration. | string | Defaults to controller-runtime's built-in default (2s) if not set or set to "". |
...
| `nginxGateway.metrics.serviceMonitor` | Defines the configuration of the serviceMonitor for the nginxGateway control plane. | object | `{"annotations":{},"enable":false,"interval":"","labels":{},"tlsConfig":{}}` |
| `nginxGateway.metrics.serviceMonitor.annotations` | Set the annotations to be added to the serviceMonitor. | object | `{}` |
| `nginxGateway.metrics.serviceMonitor.enable` | Enable the serviceMonitor for the control plane. | bool | `false` |
| `nginxGateway.metrics.serviceMonitor.interval` | The scraping interval used by the serviceMonitor when scraping metrics. | string | `""` |
| `nginxGateway.metrics.serviceMonitor.labels` | Set the labels to be added to the serviceMonitor. | object | `{}` |
| `nginxGateway.metrics.serviceMonitor.tlsConfig` | tlsConfig is an object that defines the full TLS configuration of the NGINX Gateway Fabric control plane for the Prometheus service monitor. All fields are optional and can be configured by the user. For the full information on the fields that can be used in this object, refer to the official Prometheus documentation: https://prometheus-operator.dev/docs/api-reference/api/#monitoring.coreos.com/v1.TLSConfig | object | `{}` |
...
| `nginxGateway.payloadProcessor.enable` | Enable the PayloadProcessor API. PayloadProcessors enable declarative, ordered processing of HTTP request and response payloads by attaching to a Gateway or HTTPRoute, and are used to implement features such as Guardrails for AI workloads. | bool | `false` |
