...
| `nginx.nginxInstanceManager` | Configuration for NGINX Instance Manager. | object | `{"dataplaneKeySecretName":"","endpointHost":"","endpointPort":4317}` |
| `nginx.nginxInstanceManager.dataplaneKeySecretName` | Name of the secret which holds the dataplane key that is required to authenticate with NGINX Instance Manager. Secret must exist in the same namespace that the NGINX Gateway Fabric control plane is running in (default namespace: nginx-gateway). | string | `""` |
...
| `nginxGateway.metrics.serviceMonitor` | Defines the configuration of the serviceMonitor for the nginxGateway control plane. | object | `{"annotations":{},"enable":false,"interval":"","labels":{}}` |
